...
module.exports = async (npmrc, {tarballDir, pkgRoot}, lernaPkgInfo, {cwd, env, stdout, stderr, nextRelease: {version}, logger}) => {
  ...
  const versionResult = execa(
    'npx',
    ['lerna', 'version', version, '--exact', '--include-merged-tags', '--no-git-tag-version', '--yes'],
    {
      cwd: basePath,
      env,
    }
  ...
  if (tarballDir) {
    ...
    for (const {location} of lernaPkgInfo.filter((pkg) => pkg.changed)) {
      const packResult = execa('npm', ['pack', location, '--userconfig', npmrc], {cwd, env});
