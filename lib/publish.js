...
module.exports = async (npmrc, {npmPublish, pkgRoot}, lernaPkgInfo, context) => {
  ...
  for (const pkgInfo in lernaPkgInfo.filter((pkg) => pkg.changed)) {
    logger.log(JSON.stringify(pkgInfo, null, 2));
    const pkg = getPkg(pkgInfo);
    ...
    if (npmPublish !== false && pkg.private !== true) {
      ...
      releaseInfo.push(getReleaseInfo(pkg, context, distTag, registry));
    } else {
      logger.log(
  ...
  return (releaseInfo.length > 0) ? releaseInfo: false;
};
